...
## Identifying subcommunities (i.e. the banks, in our case differnet subcommunities have no connection among them)
...
#for i in range(100,105):
    ...
    #print(most_popular_members[:10])
      

    df_out.append((i, nx.density(H), H.number_of_nodes(), most_popular_members ))
...
pd.DataFrame(df_out, columns=('community', 'density', 'n_nodes','most_popular_members'))
...
"""
###------ Example of plot of a single community : --------------------------

id_bank=214
dd=df[df.organization_emis==id_bank]

plt.rcParams['figure.figsize'] = [15, 15]

G1 = nx.from_pandas_edgelist(dd, 'account_id_emis', 'account_id_dest', ['amount', 'n_transf'])
                          
edge_widthA = [0.0001 * G1[u][v]['amount'] for u, v in G1.edges()] 
edge_widthN = [0.5 * G1[u][v]['n_transf'] for u, v in G1.edges()] 
    
nx.set_node_attributes(G1, nodeData.set_index('account_id').to_dict('index'))

node_color = [G1.degree(v) for v in G1] 

pos = nx.spring_layout(G1, scale=3)

nx.draw(G1, pos, width=edge_widthN, node_color=node_color)
node_labels = nx.get_node_attributes(G1,'special_type')
nx.draw_networkx_labels(G1,pos, node_labels)
edge_labelsA = nx.get_edge_attributes(G1,'n_transf')
nx.draw_networkx_edge_labels(G1,pos,edge_labelsA)  
